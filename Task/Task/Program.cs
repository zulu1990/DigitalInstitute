...
namespace Task
{
    internal class Program
    {
        static void Main()
        {
            ...
            Character mage2 = new Mage("Albus", 40, 20, 100, 2);
            ...
            // characters info before Attack
            foreach (Character character in characters)
            {
                Console.WriteLine(character.ToString());
            }
            warrior1.Attack(mage1);
            warrior1.Attack(warrior2); // should write that they can't Attack to each other
            warrior1.Attack(mage2);
            // characters info after warrior1 Attack
            foreach (Character character in characters)
            {
                Console.WriteLine(character.ToString());
            }
            mage1.Attack(warrior1);
            mage1.Attack(warrior2); // should write that they can't Attack to each other
            mage1.Attack(mage2);
            // characters info after mage1 Attack
            foreach (Character character in characters)
            {
                Console.WriteLine(character.ToString());
            }
            warrior2.Attack(mage2);
            Console.WriteLine(warrior2.ToString());
            Console.WriteLine(mage2.ToString());
            ...
            using(Weapon weapon=new Weapon())
            {
                mage2.Weapon = weapon;
            }
            using(Weapon weapon1=new Weapon())
            {
                Console.WriteLine(warrior2.Weapon);
                
            }

            // *************** test UseSkill

            //ISkill warrior3 = new Warrior("Evander", 20, 70, 100, ArmorType.HEAVY);
            //ISkill mage3 = new Mage("Bertram", 10, 80, 60,2);
            ...
            //foreach(ISkill charactres in skills)
            //{
            //    Console.WriteLine(charactres.ToString());
            //}
